...
async fn test_install_tools() {
    ...
    let build_dockerfile_metadata = fs::metadata(toolsdir.join("build.Dockerfile"))
        .await
        .unwrap();
    let repack_dockerfile_metadata = fs::metadata(toolsdir.join("repack.Dockerfile"))
        .await
        .unwrap();
    let buildsys_metadata = fs::metadata(toolsdir.join("buildsys")).await.unwrap();
    let build_dockerfile_mtime = FileTime::from_last_modification_time(&build_dockerfile_metadata);
    let repack_dockerfile_mtime =
        FileTime::from_last_modification_time(&repack_dockerfile_metadata);
    let buildsys_mtime = FileTime::from_last_modification_time(&buildsys_metadata);

    assert_eq!(build_dockerfile_mtime, buildsys_mtime);
    assert_eq!(repack_dockerfile_mtime, buildsys_mtime);
}
